<div align="center">

# Hi there, I'm Owis Ahmad Samih Abu Zaidan 👋

### CIS Student | Flutter Developer | Firebase Enthusiast

<img src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" width="55%" />

</div>

---

# 🚀 About Me

- 🎓 Fourth-year **Computer Information Systems (CIS)** student.
- 📱 Flutter Developer with strong experience building mobile applications.
- 🔥 Experienced with **Firebase** services using Flutter:
  - Firestore
  - Authentication
  - Cloud Messaging
  - Storage
  - Realtime Database
  - Cloud Functions
- ⚡ Building backend logic using **Firebase Cloud Functions** with **TypeScript**.
- 🌐 Experienced working with APIs and integrating external services.
- 💻 Passionate about creating clean UI and scalable mobile apps.

---

# 🛠️ Tech Stack

![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat&logo=flutter&logoColor=white)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=flat&logo=dart&logoColor=white)
![Firebase](https://img.shields.io/badge/firebase-ffca28?style=flat&logo=firebase&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-323330?style=flat&logo=javascript&logoColor=F7DF1E)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![Git](https://img.shields.io/badge/GIT-E44C30?style=flat&logo=git&logoColor=white)

---

# 📚 Currently Learning

- Advanced Flutter Architecture
- Backend Scalability
- Clean Architecture
- Performance Optimization
- Advanced Firebase Solutions

---

# 💡 Skills

```dart
class Owis {
  final String role = "Flutter Developer";

  List<String> skills = [
    "Flutter & Dart",
    "Firebase",
    "Cloud Functions",
    "TypeScript",
    "JavaScript",
    "REST APIs",
    "State Management",
    "Clean UI Design",
    "Git & GitHub"
  ];
}
```
📈 GitHub Stats
<div align="center"> <img src="https://github-readme-stats.vercel.app/api?username=owiszaidan4-del&show_icons=true&theme=tokyonight" width="47%" /> <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=owiszaidan4-del&layout=compact&theme=tokyonight" width="47%" /> </div>

📫 Contact Me
📧 Email: owiszaidan4@gmail.com
💼 LinkedIn: https://www.linkedin.com/in/owis-zaidan-b6aa4b370?utm_source=share_via&utm_content=profile&utm_medium=member_ios
